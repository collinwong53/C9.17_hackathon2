...
function View() {
  ...
  this.displayCards = function() {
    var back_image1 = $("<div>")
      .addClass("back")
      .css("background-image", "url('images/card_back.png')");
    var back_image2 = $("<div>")
      .addClass("back")
      .css("background-image", "url('images/card_back.png')");
    ...
    var player_0_front = $("<div>")
      .addClass("front")
      .css("background-image", "url(" + player0BackgroundImage + ")");
    var player_1_front = $("<div>")
      .addClass("front")
      .css("background-image", "url(" + player1BackgroundImage + ")");
    var player_0_card = $("<div>")
      .addClass("card")
      .append(player_0_front, back_image1);
    var player_1_card = $("<div>")
      .addClass("card")
      .append(player_1_front, back_image2);
  ...
  this.updateBarCounter = function() {
    $("#hp_0").text(
      game_model.players[0].hp + "/" + game_model.players[0].pokemon.hp
    );
    $("#hp_1").text(
      game_model.players[1].hp + "/" + game_model.players[1].pokemon.hp
    );
    $("#power_0").text(
      game_model.players[0].completedMoves +
        "/" +
        game_model.players[0].completedMovesGoal
    );
    $("#power_1").text(
      game_model.players[1].completedMoves +
        "/" +
        game_model.players[1].completedMovesGoal
    );
  };
  ...
  this.updateBars = function() {
    var player0HPpercentage =
      game_model.players[0].hp / game_model.players[0].pokemon.hp;
    var player1HPpercentage =
      game_model.players[1].hp / game_model.players[1].pokemon.hp;
    var player0Powerpercentage =
      game_model.players[0].completedMoves /
      game_model.players[0].completedMovesGoal;
    var player1Powerpercentage =
      game_model.players[1].completedMoves /
      game_model.players[1].completedMovesGoal;
  ...
  this.backgroundImage = function() {
    ...
    this.addImageToBackground = function() {
      var randomIndex = Math.floor(
        Math.random() * this.backgroundImageArray.length
      );
      return this.backgroundImageArray[randomIndex];
    };
    $("body").css(
      "background-image",
      "url(" + this.addImageToBackground() + ")"
    );
  };
  ...
  this.displayWinVideo = function(winnerPlayerModel, winner_video_link) {
      $(".modal-title").text(
        "Player " + parseInt(winnerPlayerModel.index + 1) + " Wins!"
      ); // The text will be the name of the pokemon
  ...
  this.get_youtube_data = function(pokemon_name) {
    var promise = {
      //create an object that will handle the promise itself
  ...
  this.winner_video = function(data) {
    var video_list = data;
    var random_vid =
      video_list.video[Math.floor(Math.random() * video_list.video.length)].id;
    var winner_video_link = "https://www.youtube.com/embed/" + random_vid;
    view.displayWinVideo(winnerPlayerModel, winner_video_link);
  };
  /***************************************************************************************************
  ...
  this.failed_video = function(message) {
    $(".modal-title").text(
      "Player " + parseInt(winnerPlayerModel.index + 1) + " Wins!"
    ); // The text will be the name of the pokemon
